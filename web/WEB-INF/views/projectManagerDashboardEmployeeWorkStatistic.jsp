...
<div class="container-fluid">
  <div class="row">
      ...
      <ul class="nav nav-sidebar">
        <li>Reports
          <ul class="nav nav-sidebar">
            ...
            <li class="active"><a href="/projectManager/chooseEmployeeForReportPage?managerId=${manager.userId}">Employee work statistic</a></li>
    ...
    <c:choose>
      <c:when test= "${fn:length(listTaskReportStatistic) > 0}">
        <h1 class="page-header">Employee ${employee.surname} ${employee.name} work statistic</h1>
        <div class="table-responsive">
          <table class="table table-striped">
            ...
            <tbody>
              <c:forEach items="${listTaskReportStatistic}" var="taskReportStatistic">
                <tr>
                  <td>${taskReportStatistic.taskName}</td>
                  <td>${taskReportStatistic.sprintName}</td>
                  <td>${taskReportStatistic.projectName}</td>
                </tr>
              </c:forEach>
            </tbody>
          </table>
      ...
      <c:otherwise>
        <h1 class="row">Employee ${employee.surname} ${employee.name} has no tasks during that period!</h1>
      </c:otherwise>
    </c:choose>
    </div>
  </div>
